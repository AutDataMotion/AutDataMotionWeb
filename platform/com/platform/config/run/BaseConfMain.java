...
public abstract class BaseConfMain {
	...
	/**
	 * <p>
	 * Title: setPropertyes<／p>
	 * <p>
	 * Description: 设置本地配置参数<／p>
	 * 
	 * @param pro
	 * @param isComm
	 * @return
	...
	public void initFtp() {
		...
		ftpTool.init((String) properityMdl.getparamMapMy(ConstantInit.ftp_ip),
				((Integer) properityMdl.getparamMapMy(ConstantInit.ftp_port))
						.intValue(), (String) properityMdl
						.getparamMapMy(ConstantInit.ftp_username),
				(String) properityMdl.getparamMapMy(ConstantInit.ftp_pwd),
				(String) properityMdl
						.getparamMapMy(ConstantInit.ftp_path_remote),
				(String) properityMdl
						.getparamMapMy(ConstantInit.ftp_path_local));

	}

	public void initFtpPool() {
		FTPClientConfigure configure = new FTPClientConfigure();
		configure.setHost((String) properityMdl
				.getparamMapMy(ConstantInit.ftp_ip));
		configure.setPort(((Integer) properityMdl
				.getparamMapMy(ConstantInit.ftp_port)).intValue());
		configure.setUsername((String) properityMdl
				.getparamMapMy(ConstantInit.ftp_username));
		configure.setPassword((String) properityMdl
		...
		try {
			this.ftpPool = new FTPClientPool(5, ftpFactory);
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
			log.error("error:initFtpPool failed");
			this.ftpPool = null;
		}

	}

	public void destroyFtpPool()  {
